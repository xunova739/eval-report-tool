...
系统支持解析多种格式的口径Excel，主要分为两大类：**线上评测模式**（日常质量评估）和 **GSB对比模式**（模型AB对比）。以下是不同模式的识别规则与解析策略。

### 5.1 线上评测模式

**适用场景**: 日常标注质量评测，评估各项指标的达标率。
...
解析结果:
  group_dimensions: [
    {"field": "评估维度", "values": ["手持物", "手指交叉", "正坐", ...]}
  ]
```

### 5.2 GSB对比模式

**适用场景**: 模型AB对比评测，评估哪个模型效果更好（Good/Same/Bad）。
...
├────────────────────────────────────────────────────────────────────┤
│                                                                    │
│  第1行: 子项     │ 左边好  │ 右边好  │ 一样好  │ 一样差  │ 总数    │
│  第2行: ─────────────────────────────────────────────────────      │
│  第3行: 子项1    │ 25      │ 30      │ 40      │ 5       │ 100     │
│  第4行: 子项2    │ ...     │ ...     │ ...     │ ...     │ ...     │
│  ...                                                               │
│                                                                    │
│  说明:                                                             │
│    - 左边好/右边好/一样好/一样差 是评测标签                          │
│    - 子项对应数据中的某个字段值                                      │
│    - 数值表示该情况的样本数                                          │
│                                                                    │
└────────────────────────────────────────────────────────────────────┘
...
| 特性 | 线上评测 | GSB对比 |
|------|----------|---------|
| 分母 | 统一或自定义 | **业务场景强相关**（如仅包含生成了衣服的有效数据） |
| 条件值 | 字段的实际值 | 固定标签(左边好/右边好等) |
| 运算符 | 多样 | 主要是contains |
| 分组 | 按字段值分组 | 按子项分组 |
| 输出 | 达标率 | 胜率/对比结果 |
...
输入口径:
```text
子项      | 左边好 | 右边好 | 一样好 | 一样差 | 总数
----------|--------|--------|--------|--------|-----
手势识别  | 25     | 30     | 40     | 5      | 100
姿态检测  | 35     | 20     | 35     | 10     | 100
```
...
{
  "common_denominator": {
    "description": "仅包含生成了衣服的评测数据",
    "type": "custom",
    "conditions": [
      {"field": "是否生成", "op": "==", "value": "是"}
    ]
  },
  "metrics": [
    {
      ...
      "numerator_conditions": [
        {"field": "评估子项", "op": "==", "value": "手势识别"},
        {"field": "评测结果", "op": "contains", "value": "左边好"}
      ],
    ...
    },
    {
      ...
      "numerator_conditions": [
        {"field": "评估子项", "op": "==", "value": "手势识别"},
        {"field": "评测结果", "op": "contains", "value": "右边好"}
      ],
      "numerator_or_conditions": [],
      "numerator_logic": "and",
      "denominator_type": "common",
