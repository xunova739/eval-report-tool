...
口径说"允许L0+L1" → 生成OR组，包含L0和L1
口径说"允许L0+L1+L2" → 生成OR组，包含L0、L1、L2

**categorical_with_multi 多选字段**的 `(L0+L1)` 通过场景：
1. 找出括号里允许的等级：如 `(L0+L1)` → L0、L1
2. 去字段分布的 options 里，找每个等级对应的**完整选项名**
...
**特殊情况（极少）：如果数据中有组合值（如"L2，L1"），OR组会同时匹配到**
- 这种情况用户可以手动改成 `not_contains L2` 来排除更高级别
- 正常数据不会有组合值，OR组是最直观的写法
