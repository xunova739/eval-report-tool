...
- 口径说"允许L0+L1" = "不允许L2及以上" = **排除所有比括号里最高级别更高的级别**
- 口径说"允许L0+L1+L2" = "不允许L3" = 排除L3

**操作步骤：**
1. 找出括号里允许的最高级别：如(L0+L1) → 最高是L1
2. 找出字段分布里所有比这个最高级别更高的级别：如字段有L2、L3 → 都要排除
3. 生成多条 not_contains 条件，排除所有更高级别

**示例：字段有 L0/L1/L2/L3，口径是"允许L0+L1"**
- 括号里最高级别是 L1
- 比 L1 更高的级别有 L2 和 L3
- ✅ 正确写法：
  ```json
  {{"field":"xxx","op":"not_contains","value":"L2xxx"}},
  {{"field":"xxx","op":"not_contains","value":"L3xxx"}}
  ```
- ❌ 错误写法：只排除 L2（会漏掉 L3）

**特殊情况：如果括号里包含了所有级别**
- 口径"允许L0+L1+L2+L3" → 没有更高级别需要排除 → 不需要生成条件（或生成空条件）
- 口径"允许L0" → 只允许L0，排除L1/L2/L3
